...
namespace OrderPurches.WebApi.Features.Auth
{
    public class AuthService
    {
        ...
        public AuthService(OrderPurchesDbContext logisticaBtdDbContext, IConfiguration configuration)
        {
        ...
        public UserDto Auth(User User)
        {
            ...
            var employee = _OrderPurchesDbContext.User
            ...
            var userPermission = _OrderPurchesDbContext.RolePermission.Where(x => x.RoleId == employee.RoleId).ToList();
            ...
            var permissions = _OrderPurchesDbContext.Permission.Where(x => permissionIds.Contains(x.PermissionId) && x.Active).ToList();

            // Se obtienen información adicional sobre el rol y el tema del usuario
            var role = _OrderPurchesDbContext.Role.Where(x => x.RoleId == employee.RoleId).FirstOrDefault();
            var theme = _OrderPurchesDbContext.Theme.Where(x => x.ThemeId == employee.ThemeId).FirstOrDefault();
